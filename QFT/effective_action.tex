...
\begin{equation}
    \varphi_J(x) := \frac{\delta W[J]}{\delta J(x)} = \E{\varphi(x)}_J.
\end{equation}
%
...
As we found in the last section, the propagator $D(x, y) = \E{\varphi(x)\varphi(y)}_J$ is given by $-i$ times the second functional derivative of $W[J]$.
...
\end{equation}
%
By assumption, $\E{\eta}_J = 0$, which means this path integral is described by only 1PI diagrams, connected or not. We can therefore write
%
\begin{equation}
...
We define the effective potential $\V_\mathrm{eff}$ by
%
\begin{equation}
    \label{definition effective potential}
    \Gamma[\varphi_0] = - V T \, \V_{\mathrm{eff}}(\varphi_0),
\end{equation}
%
...
The first term in the effective potential is modified by $\V \rightarrow g^{-1}\V$, which means that it is made up of tree-level terms.
...
\begin{equation}
    ...
    \V_\mathrm{eff}(\varphi_0) = \V(\varphi_0) - \frac{i}{VT}  \frac{1}{2} \Tr{\ln\left( - \fdv{S[\varphi_0]}{\varphi(x), \varphi(y)}  \right)}.
\end{equation}
%
